...
import { addCoins, createResearch, updateResearch } from "./database";
...
export async function increaseResearch(context: CommandContext, discordId: string, type: Research, dexId: number, currentData: any) {
    ...
    if (tasksAfter[0] > tasks[0]) {
        ...
        if (tasksAfter[0] === tasksAfter[1]) {
            ...
            await sendEmbed(context, { description: `You completed all tasks for ${pokemon.displayName}. You can claim your Shiny Ticket <:shinyticket:878363313321414676> by doing \`/research claim\`.` });
        } else {
            await sendEmbed(context, { description: `You completed a task for ${pokemon.displayName}. You received ${tasksAfter[0] * 500} Pokecoins <:pokecoin:741699521725333534>.` });
