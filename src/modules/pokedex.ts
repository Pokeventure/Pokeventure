...
import * as Pokedex from '../../simulator/dist/data/pokedex';
import * as Learnsets from '../../simulator/dist/data/learnsets';
import * as MovesImport from '../../simulator/dist/data/moves';
import experience from '../../data/experience';
import { getPokemonLocation, move } from './world';
import items, { getItemByHoldname } from '../../data/items';
import * as ItemsTextImport from '../../simulator/dist/data/text/items';
import * as MovesTextImport from '../../simulator/dist/data/text/moves';
import * as AbilitiesImport from '../../simulator/dist/data/text/abilities';
import * as NaturesImport from '../../simulator/dist/data/natures';
...
export function getMoves(id: number | string, special: string | undefined) {
...
export async function sendInfo(pokemon: IPokemon, context: CommandContext, isMarket = false, marketOffer = '', debug = false) {
  const pokemonData = getPokemon(pokemon.dexId, pokemon.special);
  const embed = new EmbedBuilder();
  //pokemon.forme = res.forme;
  const stats = getStats(pokemon.dexId, pokemon.level, pokemon.ivs, pokemon.special, pokemon.nature);
  let friendshipDisplay = '';
  for (let i = 0; i < 10; i++) {
    if (pokemon.friendship >= (i + 1) * 10) {
      friendshipDisplay += '❤️';
    } else {
      friendshipDisplay += '🖤';
    }
  ...
  const info = `${rarity[pokemon.rarity]} ${pokemon.nickname ? `${pokemon.nickname} (${pokemonData.displayName})` : pokemonData.displayName} ${pokemon.forme ? eventEmoji[pokemon.forme] : ''} ${pokemon.shiny ? '✨' : ''} ${genderEmoji[pokemon.gender] ?? ''} (lvl. ${pokemon.level})\n\n${!isMarket ? `**Friendship**:\n${friendshipDisplay} (${pokemon.friendship}/100)` : ''}`;

  let movesText = '';
  const hpType = calculateHiddenPowerType(pokemon.ivs.hp, pokemon.ivs.atk, pokemon.ivs.def, pokemon.ivs.spe, pokemon.ivs.spa, pokemon.ivs.spd);
  pokemon.moves.forEach((element: string) => {
    if (element === null) {
      return;
    }
  ...
  embed.addFields({ name: '**Moves**', value: movesText, inline: true });

  if (pokemon.item) {
    let item = getItemByHoldname(pokemon.item);
    embed.addFields({ name: '**Item**', value: `${item?.emoji}**${item?.name}**\n${ItemsText[pokemon.item].desc}`, inline: true });
  } else {
    embed.addFields({ name: '**Item**', value: `None`, inline: true });
  }
...
Sp. DEF: ${stats.spd}
SPE: ${stats.spe}
Experience: ${pokemon.experience}/${calculateLevelExperience(Math.min(100, pokemon.level + 1))}`;
  embed.addFields({ name: '**Stats**', value: statsText, inline: true });
  const nature = Natures[pokemon.nature.toLowerCase()];
  embed.addFields({ name: '**Nature**', value: `**${pokemon.nature}**\n${nature.plus !== undefined ? `${statsName[nature.plus]}+\n${statsName[nature.minus]}-` : ''}`, inline: true });
  embed.addFields({ name: '\u2800', value: '\u2800' });
  const ability = pokemon.abilitySlot ? normalizeName(getAbilities(pokemon.dexId, pokemon.special)[pokemon.abilitySlot]) || normalizeName(getAbilities(pokemon.dexId, pokemon.special)['0']) : normalizeName(getAbilities(pokemon.dexId, pokemon.special)['0']);
  if (Abilities[ability] !== undefined) {
    embed.addFields({ name: '**Ability**', value: `**${Abilities[ability].name}**\n${Abilities[ability].shortDesc}`, inline: true });
  }

  let bonusText = '';
  if (pokemon.owner !== pokemon.firstOwner) {
  ...
  if (bonusText.length > 0 && isMarket === false) {
    embed.addFields({ name: '**Bonus**', value: bonusText, inline: true });
  }
  if (!marketOffer) {
    embed.setAuthor({ name: context.user.username, iconURL: context.user.avatarURL() ?? context.user.defaultAvatarURL });
  }
  if (marketOffer) {
    embed.setTitle(`Market offer \`${marketOffer}\``);
  }
  embed.setThumbnail(getImage(pokemon, true, pokemon.shiny, pokemon.special))
    .setDescription(info);
  let footer = '';
  if (pokemon.evolutionLock) {
    footer += 'Evolution has been disabled on this Pokémon\n';
  }
  if (pokemon.mint && pokemon.mint > 0) {
    footer += `Nature mint used ${pokemon.mint} times\n`;
  }
...
export function searchPokemon(search: string) {
  ...
  console.log(fuse.search(search).map(x => x.item).slice(0, 5));
...
export {
  ...
  normalizeName, rarity,
  typeEmoji, strength, weakness,
  resist, immune,
  moveClassEmoji, getAbilities, genderEmoji,
  pokedexBase
};
