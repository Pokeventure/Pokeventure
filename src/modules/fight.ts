...
export class Fight {
    ...
    async start(context: CommandContext | ButtonContext, p1PokemonsRaw: IPokemon[], p2PokemonsRaw: IPokemon[], isRaid: number, maxHp = 0, currentHp = 0, location = -1, trainer1 = "You", trainer2 = "", isAutomatic = true, pvpSettings?: { p1: any, p2: any, broadcast: boolean }): Promise<{
        victory: number,
        inactive?: boolean,
        hp?: number,
        consumedItem?: any,
        ...
        return new Promise(async (resolve) => {
            this.inactiveTimer = debounce(120000, () => {
                this.kill();
                resolve({ victory: 0, inactive: true, turn: 0 });
            });
            ...
            if (pvpSettings?.broadcast) {
                this.broadcastMessage = await sendEmbed(context, { description: "Fight will appear here." });
            }
            ...
            function parseProtocol(text: string) {
                ...
                switch (action) {
                    case "move": {
                        ...
                        const self = move_data[2].startsWith("p1");
                    ...
                    case "-damage": {
                        ...
                        const self = damage_data[2].startsWith("p1");
                    ...
                    case "switch": {
                        ...
                        const self = switch_data[2].startsWith("p1");
                        const name = switch_data[2].split(/(p1a: |p2a: )/)[2];
                    ...
                    case "-start": {
                        ...
                        const self = start_data[2].startsWith("p1");
                    ...
                    case "-end": {
                        ...
                        const self = end_data[2].startsWith("p1");
                    ...
                    case "-activate": {
                        ...
                        const self = activate_data[2].startsWith("p1");
                    ...
                    case "-enditem": {
                        ...
                        const self = item_data[2].startsWith("p1");
                        if (item_data[4] === "[eat]") {
                            return {
                    ...
                    case "-ohko":
                        ...
                        break;
                    case "drag": {
                        const dragData = text.split("|");
                        const self = dragData[2].startsWith("p1");
                        const name = dragData[2].split(/(p1a: |p2a: )/)[2];
                        const pokemonData = dragData[3].replace(/ /g, "").split(",");
                        const gender = pokemonData.includes("M") ? "M" : pokemonData.includes("F") ? "F" : "N";
                        let level = 100;
                        if (pokemonData[1] !== undefined) {
                            if (pokemonData[1].charAt(0) === "L") {
                                level = parseInt(pokemonData[1].substr(1));
                            }
                        }
                        return {
                            text: "$NAME was dragged out!",
                            switch: {
                                displayName: pokemonData[0], name, level, shiny: !!pokemonData.includes("shiny"), gender,
                            },
                            health: dragData[4],
                            self,
                        };
                    ...
                    if (turn_result.length > 0 && turnFinished) {
                        ...
                        if (!this.isAutomatic) {
                            while (fight_result.length > 0) {
                                ...
                                embed.setColor("#0000ff")
                                    .setImage(`http://image.pokeventure.com/?d=${this.generateImage({ currentPokemonP1, currentPokemonP2 }, location, isRaid)}`)
                                    .setFooter({ text: `${text.replace(/{p1}/g, "*").replace(/{p2}/g, "").replace(/{victory p1}/, "You won!").replace(/{victory p2}/, "You lost.")}` });
                                ...
                                if (pvpSettings?.p2 !== undefined && pvpSettings?.p2 !== null) {
                                    ...
                                    embed.setFooter({ text: `${text.replace(/{p1}/g, "").replace(/{p2}/g, "*").replace(/{victory p1}/, "You lost.").replace(/{victory p2}/, "You won!")}` });
                                    updateMessage(context.client, pvpSettings?.p2.message, { embeds: [embed] });
                                }

                                if (victory !== -1) {
                                    resolve({ victory, hp: parseInt(currentPokemonP2.hp.split("/")[0]), turn: 0 });
                                }
                            }
                        }
                        turn_result = [];
