import { ButtonStyle, GuildMember, Routes, SlashCommandBuilder, User } from "discord.js";
...
export const Patreon: Command = {
    ...
    handler: async (context: CommandContext) => {
        try {
            const user: GuildMember = <GuildMember>await context.client.restClient.get(Routes.guildMember("766667446235955221", context.user.id));
            const currentPatronLevel = context.player.patronLevel || 0;
            console.log(user.roles.cache.some(role => role.name === "Mod"));
            if (!user) {
                return sendEmbed(context, {
                    description: "You can donate on Patreon to get special Perks!\n[Go on Pokeventure Patreon here](https://www.patreon.com/pokeventure)\n\nIf you subscribe, make sure to stay on the Pokeventure server to access your perks!", title: "Donate"
                }, [{
            ...
            /*if (user.roles.has("789959072249020476")) {
                // Legendary Trainer
                actualPatronLevel = 5;
